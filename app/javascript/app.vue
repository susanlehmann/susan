...
#app
  ...
  .row.mt-2
    .col-md-3.d-md-block.d-none
      .home-user.home-user__welcome.card(v-if="!currentAccount")
        .card-body
          ...
          .small
            ...
            p
              | All the code is
              a(href="https://github.com/izwx/bloblo")  open source
              |, so you can even run your own BloBlo server and use the same
              | exact account you use on this site!

            p
              | By default your messages are shared through the blockchain,
              | but BloBlo follows federated standards, so it's also compatible
      ...
      .home-user.card(v-if="currentAccount")
      ...
      div(v-if="currentAccount")
        ...
        .card
          .card-body
            textarea.change-size.form-control(placeholder="Write something on the Ethereum blockchain", v-model="newMessage")
          .card-footer  
