...
RSpec.describe AccountsHelper, type: :helper do
  describe '#link_to_account' do
    ...
    it 'returns BloBlo if matches the contract' do
      tx = Transaction.create(address: '0x1234')
      Contract.update_address :bloblo, '0x1234', tx
      account = Account.make_by_address(Contract.bloblo.address)
      expect(helper.link_to_account(account)).to eql('BloBlo Smart Contract')
    end
